  ...
  useEffect(() => {
    const fetchCategoryData = async () => {
      try {
        ...
        const categoryIds = [113, 114, 115, 116, 117, 118, 119, 120, 121, 122, 123, 124, 125]; // Add the category IDs you want to fetch
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-8 px-7 mt-16">
          {categoryDataArray.map((categoryData) => (
            <div key={categoryData.id} className="bg-white rounded-md shadow-md p-6">
               {categoryData.image && (
              <Link href={`/selected-homes/${categoryFolderMapping[categoryData.id]}`}>
              <Image
              width={1000}
              height={1000}
